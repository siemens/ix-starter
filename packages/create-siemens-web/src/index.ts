...
const { red, underline, yellow } = createColors();
...
Available templates:
element-starter          element-starter`
...
const defaultTargetDir = 'siemens-web-app';
...
async function init() {
  ...
  if (isAgent && interactive) {
    console.log(
      '\nTo create in one go, run: create-siemens-web <DIRECTORY> --no-interactive --template <TEMPLATE>\n',
    );
  }
